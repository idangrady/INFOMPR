...
def parse_story_file(content):
    ...
    """
    splitted_content = content.split("@highlight")
    content_raw = splitted_content[0]
    content = " ".join(filter(None, [x.strip() for x in content_raw.split("\n")]))
    references_raw = splitted_content[1:]
    references = [" ".join(filter(None, [x.strip() for x in ref.split("\n")])) for ref in references_raw]
...
def annotation_pairing(args):
    ...
    for example in dataset:
        ...
        with open(story_path) as fd:
            ...
            example["text"], example["references"] = parse_story_file(story_content)
