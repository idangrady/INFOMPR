...
SIZE_OF_DATASET_IN_ARTICLES = 15000
MAX_ARTICLE_LEN = 400
MAX_ABSTRACT_LEN = 30
...
for i in news_data['Cleaned_Abstract']:
    if(len(i.split())<=MAX_ABSTRACT_LEN):
        count=count+1
print(f"% of articles that are >= {MAX_ABSTRACT_LEN - 10} and <= {MAX_ABSTRACT_LEN} tokens: {(count/len(news_data['Cleaned_Abstract'])) * 100}")
...
for i in range(len(cleaned_article)):
    abstract_len = len(cleaned_abstract[i].split())
    article_len = len(cleaned_article[i].split())
    if(abstract_len >= (MAX_ABSTRACT_LEN - 10) and abstract_len <= MAX_ABSTRACT_LEN and article_len <=MAX_ARTICLE_LEN):
...
print(f"We have kept {len(short_article)} articles")
...
#Size of vocabulary
# we only keep num_words -1; so num_words MINUS 1 is actually the size of the vocabulary
# X_voc should be just num_words because we have num_words - 1 words and using masking in the embedding
# requires the vocabulary size + 1
X_voc   =  X_tokenizer.num_words
print(f"Size of article vocabulary: {X_voc}")
...
print(f"Size of training vocabulary: {y_voc}")
...
# TODO: try smaller dimensions as well
latent_dim = 256
embedding_dim = 256
...
#Dense layer
# TODO: if units = y_voc, we allow to predict SOS as well as Padding; do we want that? padding makes sense but SOS?
...
print(f"batch size: {int(y_train.shape[0] * 0.01)}")
history = model.fit(
    x = [X_train,y_train[:,:-1]], 
    y = y_train.reshape(y_train.shape[0],y_train.shape[1], 1)[:,1:], 
    epochs=20,callbacks=[es],batch_size = int(y_train.shape[0] * 0.01), 
